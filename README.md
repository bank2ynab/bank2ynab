...
1. Run the `bank2ynab.py` conversion script to generate the YNAB-ready CSV output file. How to do this depends on your operating system:
 ...
 1. Depending on your configuration, the conversion script will now import your files into YNAB automatically or you can add the files manually:
    - Automatic import (needs an YNAB API access token to work): 
      - The conversion script will now ask you which budget it should use to import your converted CSV file to. It will also ask you which account inside the budget to use (if you have multiple). You will only have to answer these question the first time you use an account.
